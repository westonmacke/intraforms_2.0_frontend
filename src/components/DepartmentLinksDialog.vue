      ...
      <v-card-text class="pt-4">
        ...
        >
          ...
          <template v-slot:item.departments="{ item }">
            <v-chip
              v-for="deptId in item.departmentIds"
              :key="deptId"
              size="small"
              class="mr-1"
            >
  ...
  <v-dialog v-model="editDialog" max-width="600px">
    <v-card>
      <v-card-title class="bg-grey-lighten-4">
        ...
        {{ editingLink.id ? 'Edit' : 'Add' }} Department Link
      </v-card-title>
      ...
      <v-card-text class="pt-4">
        ...
        <v-select
          v-model="editingLink.departmentIds"
          :items="departments"
          item-title="name"
          item-value="id"
          ...
          multiple
          chips
          :rules="[v => v && v.length > 0 || 'At least one department is required']"
          hint="Select which departments can see this link"
          persistent-hint
        ></v-select>
      </v-card-text>
...
const fetchLinks = async () => {
  loading.value = true
  try {
    const response = await api.get('/departmentlinks/all')
...
const saveLink = async () => {
  if (!editingLink.value.title || !editingLink.value.url || !editingLink.value.departmentIds?.length) {
    return
  }
  ...
  try {
    if (editingLink.value.id) {
      ...
      await api.put(`/departmentlinks/${editingLink.value.id}`, editingLink.value)
    } else {
      // Create new link
      await api.post('/departmentlinks', editingLink.value)
    }
    editDialog.value = false
    fetchLinks()
    emit('refresh')
  } catch (error) {
    console.error('Failed to save department link:', error)
  } finally {
    saving.value = false
  }
}
...
const deleteLink = async () => {
  ...
  deleting.value = true
  try {
    await api.delete(`/departmentlinks/${deletingLink.value.id}`)
    deleteDialog.value = false
    fetchLinks()
    emit('refresh')
  } catch (error) {
    console.error('Failed to delete department link:', error)
  } finally {
...
const reorderLinks = async (newLinks) => {
  try {
    ...
    await api.post('/departmentlinks/reorder', { linkIds })
    fetchLinks()
    emit('refresh')
  } catch (error) {
    console.error('Failed to reorder department links:', error)
  }
}
